...
interface XMLManifest {
  PackageManifest: {
    ...
    Metadata: Array<{
      ...
      DisplayName: string[];
      Identity: Array<{
        $: { Id: string; Version: string; Publisher: string };
      }>;
      Tags: string[];
      GalleryFlags: string[];
      License: string[];
      Icon: string[];
      Properties: Array<{
        Property: Array<{ $: { Id: string; Value: string } }>;
      }>;
      Categories: string[];
      Badges: Array<{
        Badge: Array<{
    ...
    Installation: Array<{ InstallationTarget: Array<{ $: { Id: string } }> }>;
    Dependencies: string[];
    Assets: Array<{ Asset: Array<{ $: { Type: string; Path: string } }> }>;
  };
}

interface ContentTypes {
  Types: {
    Default: Array<{ $: { Extension: string; ContentType: string } }>;
  };
}
...
function _toVsixManifest(manifest: Manifest, files: File[]): Promise<string> {
...
function assertProperty(
  ...
  const property = manifest.PackageManifest.Metadata[0].Properties[0].Property.filter(
    p => p.$.Id === name
  );
  ...
  assert.equal(
    ...
    value,
    `Property '${name}' should have value '${value}'`
  );
}
...
describe('collect', function() {
  ...
  it('should ignore **/.vsixmanifest', () => {
    ...
    return readManifest(cwd)
      ...
      .then(files => {
        assert.equal(
          files.filter(f => /\.vsixmanifest$/.test(f.path)).length,
          1
        );
      });
  });
  ...
  it('should skip all node_modules when dependencyEntryPoints is []', () => {
    ...
    return readManifest(cwd)
      .then(manifest =>
        collect(manifest, { cwd, useYarn: true, dependencyEntryPoints: [] })
      )
      .then(files => {
        files.forEach(file =>
          assert.ok(file.path.indexOf('/node_modules/which/') < 0, file.path)
        );
      });
  });
});
  ...
  it('should not patch NLS if required', () => {
    ...
    return readManifest(cwd, false).then((manifest: any) => {
      ...
      assert.notEqual(
        ...
        translations['extension.description']
      );
      assert.notEqual(
        manifest.contributes.debuggers[0].label,
        translations['node.label']
        ...
        validateManifest(
          createManifest({
            badges: [
              {
                url:
                  'https://dev.w3.org/SVG/tools/svgweb/samples/svg-files/410.sv%67',
                href: 'http://badgeurl',
                description: 'this is a badge',
              },
            ],
...
describe('toVsixManifest', () => {
  it('should produce a good xml', () => {
    ...
    return _toVsixManifest(manifest, [])
      ...
      .then(result => {
        ...
        assert.equal(
          (result.PackageManifest as any).$['xmlns:d'],
        ...
        assert.equal(
          result.PackageManifest.Metadata[0].Description[0]._,
          'test extension'
        );
        assert.equal(result.PackageManifest.Metadata[0].DisplayName[0], 'test');
        assert.equal(
          result.PackageManifest.Metadata[0].Identity[0].$.Id,
          'test'
        );
        assert.equal(
          result.PackageManifest.Metadata[0].Identity[0].$.Version,
          '0.0.1'
        );
        assert.equal(
          result.PackageManifest.Metadata[0].Identity[0].$.Publisher,
          'mocha'
        );
        assert.deepEqual(result.PackageManifest.Metadata[0].Tags, ['']);
        assert.deepEqual(result.PackageManifest.Metadata[0].GalleryFlags, [
          'Public',
        ]);
        assert.equal(result.PackageManifest.Installation.length, 1);
        assert.equal(
          result.PackageManifest.Installation[0].InstallationTarget.length,
          1
        );
        assert.equal(
        ...
        assert.equal(
          result.PackageManifest.Assets[0].Asset[0].$.Path,
          'extension/package.json'
        );
      });
  });
  ...
  it('should treat README.md as asset', () => {
    ...
    return _toVsixManifest(manifest, files)
      ...
      .then(result => {
        ...
        assert.equal(
          result.PackageManifest.Assets[0].Asset[1].$.Path,
          'extension/readme.md'
        );
      });
  });
  ...
  it('should respect display name', () => {
    ...
    return _toVsixManifest(manifest, [])
      ...
      .then(result => {
        ...
        assert.equal(
          result.PackageManifest.Metadata[0].DisplayName[0],
          'Test Extension'
        );
      });
  });
  ...
  it('should add a license metadata tag', () => {
    ...
    return _toVsixManifest(manifest, files)
      ...
      .then(result => {
        ...
        assert.equal(
          result.PackageManifest.Metadata[0].License[0],
          'extension/thelicense.md'
        );
      });
  });
  ...
  it('should add an icon metadata tag', () => {
    ...
    return _toVsixManifest(manifest, files)
      ...
      .then(result => {
        ...
        assert.equal(
          result.PackageManifest.Metadata[0].Icon[0],
          'extension/fake.png'
        );
        assert.equal(
          result.PackageManifest.Metadata[0].License[0],
  ...
  it('should understand gallery color and theme', () => {
    ...
    return _toVsixManifest(manifest, [])
      .then(xml => parseXmlManifest(xml))
      .then(result => {
        const properties = result.PackageManifest.Metadata[0].Properties[0].Property.map(
          p => p.$
        );
        assert.ok(
          properties.some(
            p =>
              p.Id === 'Microsoft.VisualStudio.Services.Branding.Color' &&
              p.Value === '#5c2d91'
          )
        );
        assert.ok(
          properties.some(
            p =>
              p.Id === 'Microsoft.VisualStudio.Services.Branding.Theme' &&
              p.Value === 'dark'
          )
        );
      });
        ...
        assert.ok(
          properties.some(
            p =>
              ...
              p.Value ===
                'https://server.com/Microsoft/vscode-spell-check/issues'
          )
        );
        assert.ok(
          properties.some(
  ...
  it('should detect github repositories', () => {
    ...
    return _toVsixManifest(manifest, [])
      ...
      .then(result => {
        ...
        assert.ok(
          properties.every(
            p => p.Id !== 'Microsoft.VisualStudio.Services.Links.Repository'
          )
        );
      });
  });
  ...
  it('should not automatically add theme tag when themes are empty', () => {
    ...
    return _toVsixManifest(manifest, [])
      .then(parseXmlManifest)
      .then(result =>
        assert.deepEqual(result.PackageManifest.Metadata[0].Tags[0], '')
      );
  });
  ...
  it('should automatically add theme tag for icon themes', () => {
    const manifest = {
      ...
      contributes: {
        iconThemes: [
          { id: 'fakeicons', label: 'fakeicons', path: 'fake.icons' },
        ],
      },
    };
  ...
  it('should automatically add language tag with activationEvent', () => {
    ...
    return _toVsixManifest(manifest, [])
      .then(parseXmlManifest)
      .then(result =>
        assert.deepEqual(result.PackageManifest.Metadata[0].Tags[0], 'go')
      );
  });
  ...
  it('should automatically add snippets tag', () => {
    ...
    return _toVsixManifest(manifest, [])
      .then(parseXmlManifest)
      .then(result =>
        assert.deepEqual(result.PackageManifest.Metadata[0].Tags[0], 'snippet')
      );
  });
  ...
  it('should remove duplicate tags', () => {
    ...
    return _toVsixManifest(manifest, [])
      .then(parseXmlManifest)
      .then(result =>
        assert.deepEqual(result.PackageManifest.Metadata[0].Tags[0], 'theme')
      );
  });
  ...
  it('should expose localization contributions as assets', () => {
    ...
    return _toVsixManifest(manifest, files)
      ...
      .then(result => {
        ...
        const properties =
          result.PackageManifest.Metadata[0].Properties[0].Property;
    ...
    return _toVsixManifest(manifest, [])
      ...
      .then(result => {
        ...
        const engineProperties = properties.filter(
          p => p.$.Id === 'Microsoft.VisualStudio.Code.Engine'
        );
  ...
  it('should include extra extensions', () => {
    ...
    return toContentTypes(files)
      ...
      .then(result => {
        ...
        assert.ok(
          result.Types.Default.some(
            d => d.$.Extension === '.txt' && d.$.ContentType === 'text/plain'
          ),
          'there are txt'
        );
        assert.ok(
          result.Types.Default.some(
            d => d.$.Extension === '.png' && d.$.ContentType === 'image/png'
          ),
          'there are png'
        );
        assert.ok(
          result.Types.Default.some(
            d =>
              d.$.Extension === '.md' &&
              /^text\/(x-)?markdown$/.test(d.$.ContentType)
          ),
          'there are md'
        );
...
describe('MarkdownProcessor', () => {
  ...
  it('should take baseContentUrl', () => {
    ...
    return processor
      ...
      .then(file => read(file))
      .then(actual => {
        return readFile(path.join(root, 'readme.expected.md'), 'utf8').then(
          expected => {
  ...
  it('should replace img urls with baseImagesUrl', () => {
    ...
    return processor
      ...
      .then(actual => {
        return readFile(
          path.join(root, 'readme.images.expected.md'),
          'utf8'
        ).then(expected => {
          assert.equal(actual, expected);
        });
  ...
  it('should replace issue links with urls if its a github repo.', () => {
    ...
    return processor
      ...
      .then(actual => {
        return readFile(
          path.join(root, 'readme.github.expected.md'),
          'utf8'
        ).then(expected => {
          assert.equal(actual, expected);
        });
  ...
  it('should not replace issue links with urls if its not a github repo.', () => {
    ...
    return processor
      ...
      .then(file => read(file))
      .then(actual => {
        return readFile(path.join(root, 'readme.github.md'), 'utf8').then(
          expected => {
            assert.equal(actual, expected);
          }
        );
